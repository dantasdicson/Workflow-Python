...
async function callUpstream(req, access, pathParts) {
  const path = Array.isArray(pathParts) ? pathParts.join('/') : ''
  const url = new URL(`${API_BASE_URL}/api/ordens/${path}`)
...
export default async function handler(req, res) {
  ...
  let upstream = await callUpstream(req, access, pathParts)
  ...
  if (upstream.status === 401 && refresh) {
    ...
    if (newAccess) {
      ...
      access = newAccess
      upstream = await callUpstream(req, access, pathParts)
    }
  }
